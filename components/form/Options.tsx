...
const renderStepContent = (
  ...
  switch(activeStep) {
    case 0:
      return (<QuestionFormatParameters formId={formId} formField={formField} />);
    case 1: 
      return (<QuestionHolder formModel={formModel} formValues={values} setValues={setValues}/>)
    case 2:
...
function Options() {
  ...
  const submitForm = async (values: IFormValues, actions: FormikHelpers<IFormValues>) => {
    const { isMultiPage, numberOfStudents, questionHolder, studentIdTextFile } = values;
    ...
    if (studentIdTextFile) {
      formData.append("studentIdTextFile", studentIdTextFile, studentIdTextFile.name);
    }
          ...
          { ({ values, isSubmitting, setValues }) => (
            <Form id={formId}>
              ...
              <div className={classes.buttonDiv}>
                  ...
                  <Button
                    disabled={isSubmitting || (activeStep == 1 && values.questionHolder.length < 2)}
                    type="submit"
                    variant="contained"
                    color="primary"
